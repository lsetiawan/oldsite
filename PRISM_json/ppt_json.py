...
def create_vars(variable, files, ppt_mins,ppt_max,tmean_mins,tmean_max,month, path):
    # PRISM data
    ppt_tick1 = ["0.0", "40.0", "80.0", "120.0", "160.0", "> 160.0"]
    ppt_units1 = "Millimeters (mm)"
    ppt_tick2 = ["0.0", "1.6", "3.1", "4.7", "6.3", "> 6.3"]
    ppt_units2 = "Inches (in)"
    
    tmean_tick1 = ["< -10.0", "-2.0", "6.0", "14.0", "22.0", "> 30.0"]
    tmean_units1 = "Celcius (C)"
    tmean_tick2 = ["< 28.4", "28.4", "42.8", "57.2", "71.6", "> 71.6"]
    tmean_units2 = "Celcius (F)"
    ...
    if variable == "Precipitation":
        ...
        units1 = ppt_units1
        colorbar_url = os.path.join(path,"colorbar/ppt_colorbar.png")
    ...
    if variable == "Mean Air Temperature":
        ...
        units1 = tmean_units1
        colorbar_url = os.path.join(path,"colorbar/tmean_colorbar.png")
    ...
    month = month
    bounds = [-125.021,24.062,-66.479,49.938]
    image_url = os.path.join(os.getcwd(),files)
    variable = variable
    same_version = True
...
def getminmax(prop):
    ...
    if prop == "ppt":
        os.chdir('/Users/lsetiawan/Documents/APL/PRISM/ppt_climatology')
    ...
    if prop == "tmean":
        os.chdir('/Users/lsetiawan/Documents/APL/PRISM/tmean_climatology')
        for files in os.listdir("."):
            if ".tif" in files and ".aux.xml" not in files and ".DS_Store" not in files:
                src = gdal.Open(files)
                raw_data = src.GetRasterBand(1)
...
def main():
    ...
    # /Users/lsetiawan/Documents/APL/PRISM/PRISM_png
    ppt_climatology = []
    tmean_climatology = []
    month = ""
    ...
    print(json.dumps(ppt_climatology, sort_keys=True))
    print(json.dumps(tmean_climatology, sort_keys=True))
